...
def download_youtube(url, dirname, video_id) -> str:
    cmd = f"{youtube_downloader} \"{{}}\" --print after_move:filepath -o \"{{}}%(id)s.%(ext)s\""
...
def download_aslpro(url, dirname, video_id) -> str:
...
def download_others(url, dirname, video_id) -> str:
...
def download_video(url, dirname, video_id) -> str:
    download = select_download_method(url)
    video_path = ""

    try:
        video_path = download(url, dirname, video_id)
    except Exception as e:
        # logging.error(f"Unsuccessful downloading - video {video_id}")
        return None
